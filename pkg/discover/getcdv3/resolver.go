...
func NewResolver(schema, etcdAddr, serviceName string, operationID string) (*Resolver, error) {
	etcdCli, err := clientv3.New(clientv3.Config{
		...
		Username:  global.GVA_CONFIG.Etcd.UserName,
		Password:  global.GVA_CONFIG.Etcd.Password,
	})

	if err != nil {
		global.GVA_LOG.Error("etcd client v3 failed", zap.String("operationID", operationID), zap.Error(err))
		return nil, err
	}
	...
	conn, err := grpc.DialContext(ctx, GetPrefix(schema, serviceName),
		...
		grpc.WithInsecure())
	global.GVA_LOG.Debug("", zap.String("operationID", operationID), zap.String("etcd key", GetPrefix(schema, serviceName)))
	if err == nil {
...
func getConn(schema, etcdaddr, serviceName string, operationID string) *grpc.ClientConn {
	...
	if ok {
		global.GVA_LOG.Debug("", zap.String("operationID", operationID), zap.String("etcd key", schema+serviceName), zap.Any("key value", *r.grpcClientConn), zap.Any("r", *r))
	...
	if err != nil {
		global.GVA_LOG.Error("etcd failed", zap.String("operationID", operationID), zap.String("schema", schema), zap.String("etcdaddr", etcdaddr), zap.String("serviceName", serviceName), zap.Error(err))
		rwNameResolverMutex.Unlock()
		return nil
	}
...
func GetConfigConn(serviceName string, operationID string) *grpc.ClientConn {
	...
	// 	if err != nil {
	// 		global.GVA_LOG.Error("GetLocalIP failed", zap.Error(err))
	// 		return nil
	// 	}
	// }
	...
	// 	global.GVA_LOG.Error("len(configPortList) == 0 ", zap.String("operationID", operationID), zap.Error(err))
	// 	return nil
	// }
	// target := rpcRegisterIP + ":" + utils.Int32ToString(int32(configPortList[0]))
	// global.GVA_LOG.Info("", zap.String("operationID", operationID), zap.String("rpcRegisterIP", rpcRegisterIP), zap.Any("port", configPortList), zap.Any("grpc target", target), zap.Any("serviceName", serviceName))
	// conn, err := grpc.Dial(target, grpc.WithInsecure())
	// if err != nil {
	// 	global.GVA_LOG.Error("grpc.Dail failed", zap.String("operationID", operationID), zap.Error(err))
	// 	return nil
	// }
	// return conn
	return nil
...
func (r *Resolver) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
	...
	if err == nil {
		...
		for i := range resp.Kvs {
			global.GVA_LOG.Debug("", zap.String("etcd init addr", string(resp.Kvs[i].Value)))
...
func (r *Resolver) watch(prefix string, addrList []resolver.Address) {
	...
	for n := range rch {
		...
		for _, ev := range n.Events {
			...
			case mvccpb.PUT:
				if !exists(addrList, string(ev.Kv.Value)) {
					...
					global.GVA_LOG.Debug("after add, new list", zap.Any("addrList", addrList))
				}
			case mvccpb.DELETE:
				global.GVA_LOG.Debug("remove addr key", zap.Any("key", string(ev.Kv.Key)), zap.Any("value", string(ev.Kv.Value)))
				i := strings.LastIndexAny(string(ev.Kv.Key), "/")
				if i < 0 {
					return
				}
				t := string(ev.Kv.Key)[i+1:]
				global.GVA_LOG.Debug("remove addr key", zap.Any("key", string(ev.Kv.Key)), zap.Any("value", string(ev.Kv.Value)), zap.Any("addr", t))
				if s, ok := remove(addrList, t); ok {
					flag = 1
					addrList = s
					global.GVA_LOG.Debug("after remove, new list", zap.Any("addrList", addrList))
				}
			}
		}
		...
		if flag == 1 {
			r.cc.UpdateState(resolver.State{Addresses: addrList})
			global.GVA_LOG.Debug("update", zap.Any("addrList", addrList))
		}
	}
}
...
// 		global.GVA_LOG.Info("", zap.String("operationID", operationID), zap.String("rpcRegisterIP", rpcRegisterIP), zap.Any("port", configPortList), zap.Any("target", target))
// 		conn, err := grpc.Dial(target, grpc.WithInsecure())
// 		if err != nil {
// 			global.GVA_LOG.Error("grpc.Dail failed", zap.String("operationID", operationID), zap.Error(err))
// 			continue
...
func getConn4Unique(schema, etcdaddr, servicename string) []*grpc.ClientConn {
	...
	if err != nil {
		global.GVA_LOG.Error("lientv3.New failed", zap.Error(err))
		return nil
	}
